 ...
 * 
 */
package com.fph.appnavigationlib.log;
...
/**
 *
 * com.fph.OkHttpLogInterceptor.java
 * 
 * Created by wang on 2016年6月21日下午2:47:07
 * 
 * Tips:
 */
public class OkHttpLogInterceptor implements okhttp3.Interceptor {

	public static final String TAG = "NetLog";
	private String tag;
	private boolean showResponse=false;

	public OkHttpLogInterceptor(String tag, boolean showResponse) {
	...
	public OkHttpLogInterceptor(String tag) {
		this(tag, true);
	}
				...
				if (body != null) {
					...
					if (mediaType != null) {
						...
						if (isText(mediaType)) {
							...
							String copyResp = new String(resp);
							Log.e(tag, "responseBody's content : " );
							while (isLarge) {
								if (copyResp.length() > 4000) {
									String string = copyResp.substring(0, 3999);
									Log.e( tag,string);
									copyResp =copyResp.replace(string, "");
								}else{
									Log.e(tag, copyResp);
									isLarge=false;
								}
