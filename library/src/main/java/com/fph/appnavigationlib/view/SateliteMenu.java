...
import com.fph.appnavigationlib.listener.OnNumberKeyboard;
...
public class SateliteMenu extends ViewGroup implements View.OnClickListener {
	...
	public OnNumberKeyboard onNumberKeyboard = null;
	...
	public OnNumberKeyboard getOnNumberKeyboard() {
		return onNumberKeyboard;
	}
	...
	public SateliteMenu(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		int position = typedArray.getInt(R.styleable.SateliteMenu_satelitemenuposition, LEFT_TOP);
		...
		case RIGHT_BOTTOM:
			...
			break;
		case RIGHT_CENTER:
			mPosition = Position.POS_RIGHT_CE;
			break;
		}
		mRadius = (int) typedArray.getDimension(R.styleable.SateliteMenu_satelitemenuradius, defRadius);

		mBo= (int) typedArray.getDimension(R.styleable.SateliteMenu_satelitemenuradmar, defRadius);

		typedArray.recycle(); //回收
		mStatus = STATUS_CLOSE; //默认关闭状态
		onNumberKeyboard=new OnNumberKeyboard() {
		...
		onNumberKeyboard.OnSetNumber(mStatus);
//		this.setOnClickListener(this);
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {//l=0, t=0  因为是相对于父view的位置
		 ...
		 */
		int count = getChildCount();
		double angle = 180.0f / (count - 2);//这里-2，是多减去了一个menuButton
		View child;
		int w,h;
	...
	public void onClick(View v) {
		...
		rotateMenuBotton(mMenuButton, 360, 0);
		toggleMenu(500);
	...
	private void toggleMenu(int duration) {
		...
		for (int i = 1; i < count; i++) {
			...
			double angle = 180 / (count - 2);
			/*
             一个圆的弧度是2π,角度是360°   π/2即90度的弧度
			 */
			int oppositeLen = (int) (mRadius * Math.sin(Math.PI / (count - 2) * (i - 1))); //对边 横向
			int adjacentLen = (int) (mRadius * Math.cos(Math.PI / (count - 2) * (i - 1))); //邻边 纵向
			/*
